...
export default function UserTypeToggle({ variant = 'full' }: UserTypeToggleProps) {
  ...
  if (variant === 'compact') {
    return (
      <div className="user-toggle user-toggle-compact border border-green-500/20 bg-black/60 px-2 py-2 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setUserType('human')}
            className={`user-toggle-btn px-3 py-1 text-[10px] uppercase tracking-[0.22em] transition-all ${
              userType === 'human'
          ...
          {userType === 'human' ? (
            <div className="hidden md:flex items-center gap-3 text-[10px] uppercase tracking-[0.22em]">
              <Link href="/about" className="user-toggle-link text-green-400/70 hover:text-green-300">
                About
              </Link>
              <Link href="/docs" className="user-toggle-link text-green-400/70 hover:text-green-300">
                Docs
              </Link>
              <Link href="/architecture" className="user-toggle-link text-green-400/70 hover:text-green-300">
                Stack
              </Link>
            </div>
          ) : (
            <button
              onClick={handleCopy}
              className="user-toggle-copy hidden md:inline-flex items-center gap-2 overflow-hidden whitespace-nowrap border border-cyan-500/30 bg-cyan-500/10 px-3 py-1 text-[10px] text-cyan-300 transition-all hover:bg-cyan-500/15"
            >
  ...
  return (
    <div className="user-toggle user-toggle-full relative max-w-[40rem] border border-green-500/30 bg-black/65 p-5 backdrop-blur-md">
      ...
      <div className="relative z-10">
        <div className="user-toggle-tabs mb-4 flex gap-2 border border-green-500/20 bg-black/40 p-1.5">
          <button
            onClick={() => setUserType('human')}
            className={`user-toggle-btn flex-1 px-4 py-2 text-xs uppercase tracking-[0.28em] transition-all ${
              userType === 'human'
        ...
        {userType === 'human' ? (
          <div className="space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div className="user-toggle-card border border-green-500/20 bg-black/40 p-3">
          ...
          <div className="space-y-4">
            ...
            <div className="user-toggle-copy border border-cyan-500/30 bg-black/80 p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <code className="overflow-x-auto text-sm text-cyan-300">{skillCommand}</code>
                <button
                  onClick={handleCopy}
                  className="user-toggle-btn border border-cyan-500/30 bg-cyan-500/10 px-4 py-2 text-xs uppercase tracking-[0.22em] text-cyan-300 transition-all hover:bg-cyan-500/15"
                >
                  {copied ? 'Copied' : 'Copy'}
                </button>
              </div>
